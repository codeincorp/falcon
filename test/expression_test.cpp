...
TEST(ExpressionTests, HashExpressionTest)
{
    Metadata metadata{
        {"a", tiInt},
        {"b", tiUint},
    ...
    vector<any> data{"abcd"s, 1, 3.3, 1.1f, 3u};
    ...
    uint64_t hval = 0;
    auto intHval = hash<int>()(1);
    EXPECT_EQ(intHval, any_cast<uint64_t>(expr.eval(metadata, data)));
    ...
    expr = {
        .opCode = OpCode::Hash,
        .leafOrChildren = vector<Expression>{
            {.opCode = OpCode::Const, .leafOrChildren = std::any(3u)},
        }
    };

    auto uintHval = hash<unsigned>()(3u);
    EXPECT_EQ(uintHval, any_cast<uint64_t>(expr.eval(metadata, data)));

    hval ^= uintHval;

    // expression: hash(1.1f)
    expr = {
        .opCode = OpCode::Hash,
        .leafOrChildren = vector<Expression>{
            {.opCode = OpCode::Const, .leafOrChildren = std::any(1.1f)},
        }
    };

    auto floatHval = hash<float>()(1.1f);
    EXPECT_EQ(floatHval, any_cast<uint64_t>(expr.eval(metadata, data)));
    ...
    expr = {
        .opCode = OpCode::Hash,
        .leafOrChildren = vector<Expression>{
            {.opCode = OpCode::Const, .leafOrChildren = std::any(3.3)},
        }
    };

    auto doubleHval = hash<double>()(3.3);
    EXPECT_EQ(doubleHval, any_cast<uint64_t>(expr.eval(metadata, data)));

    hval ^= doubleHval;

    // expression: hash("abcd")
    expr = {
        .opCode = OpCode::Hash,
        .leafOrChildren = vector<Expression>{
            {.opCode = OpCode::Const, .leafOrChildren = std::any("abcd"s)},
        }
    };

    auto strHval = hash<string>()("abcd"s);
    EXPECT_EQ(strHval, any_cast<uint64_t>(expr.eval(metadata, data)));
